...
const subsidies: SubsidyData[] = [
  {
    ...
    note: "本页金额·补助率为令和8年度口径（较令和7无变化）。省力化补助金为审查採択制、公募回制，提交申请不保证一定採択。令和8年度第 7 回受付自 2026 年 7 月 1 日起；且第 7 回起对「员工 21 人以上」新增「一般事業主行動計画の公表」为必须要件。具体受付締切以官方申请マイページ及最新公募要領为准。",
  },
  {
    slug: "ai-it",
    tag: "AI导入",
    ...
    verifiedDate: "2026-08-01",
    note: "本补助金为审查採択制（旧称 IT 导入补助金），需经登录的 IT 导入支援事业者共同申请，有公募締切。2026 年度枠构成·补助率较令和7无变化；交付申请自 2026 年 3 月 30 日起，截至核验日最新可申请为通常枠第 4 次締切 2026 年 8 月 25 日 17:00（此后按月轮次）。金额·締切以官方最新公募要領为准，提交申请不保证採択。",
  },
  {
    slug: "career-up",
    tag: "助成金",
    ...
    verifiedDate: "2026-08-01",
    note: "本页金额为中小企业·令和8年度版口径（金额较令和7无变化）。令和8年度起キャリアアップ計画由事前认定简化为「届出のみ」；并新设「情报公表加算」——将正规转换制度概要与实绩公表于自社官网或厚労省「しょくばらぼ」，可加算 1 事业所 20 万円（大企业 15 万円·1 回限）。属要件满足原则支给的助成金（非竞争審査制），以厚生労働省最新案内为准。",
  },
  {
    slug: "training",
    tag: "人材育成",
    ...
    verifiedDate: "2026-08-01",
    note: "「1 亿円」仅为「事業展開等リスキリング支援コース」的 1 事业所年度上限，一般 AI/IT 训练走人材育成支援コース（1,000 万）或人への投資促進コース（2,500 万）。⚠️ 重要时限：事業展開等リスキリング支援コース与人への投資促進コース为令和 4〜8 年度期间限定，令和 8 年度（至 2027 年 3 月 31 日）为最终年度，官方尚未公布延长——有意活用请务必在本年度内提出计画届。属要件满足原则支给的助成金，金额以厚生労働省最新案内为准（已核 2026 令和8年度口径，金额较令和7无变化）。",
  },
  {
    slug: "aircon",
    tag: "节能补助",
    ...
    verifiedDate: "2026-08-01",
    note: "本页为东京都令和8年度口径（助成率与上限三档金额较令和7无变化）。令和8年度受付由年 5 回增至 6 回、事业期间延长至令和 11 年度、预算约 102 亿円。回次制·抽签（超预算时对受付期间内全件抽签，非先着顺）。截至核验日第 3 回受付期为 7/31〜8/14。东京都以外地区及国家层面的省エネ补助制度各异，需按所在地单独核实。金额·助成率·受付期以主管机关最新公告为准。",
  },
];
// ─────────────────────────────────────────────
