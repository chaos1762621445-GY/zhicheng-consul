...
// 按标题语义自动判定内容类型（对应用户决策阶段三分类 + 三套模板）。
// 新扩话题池无需手工打标签，标题含判定词即自动归类。
function inferType(topic) {
  const t = topic.title;
  // 对比/评测型（决策阶段：用户在比较选择）
  if (/vs|VS|还是|区别|对比|怎么选|哪个|哪些更|TOP\d|最值得|该找谁|该选|比较/.test(t))
    return "compare";
  // 问题解决型（用户遇到具体痛点/风险，要结论+步骤）
  if (/怎么办|怎么写|怎么做|怎么准备|如何应对|失败|驳回|被骗|骗局|黑中介|风险|返还|退还|注意事项|应对|踩的?坑|避坑|被要求|义务/.test(t))
    return "solve";
  // 其余=终极指南型（认知阶段：制度解读/行业盘点/资格科普）
  return "guide";
}

// 三套 AI 友好型内容模板（对应用户方法：终极指南 / 横向评测 / 问题解决）
function buildStructure(type) {
  if (type === "compare") {
    return `【本篇为「横向评测型」内容——用户正在比较选择，帮他做决策】
结构要求：
...
4. 结尾H2「怎么选？按你的情况对号入座」：按企业规模/预算/行业/紧急度分3-4种典型情况，各给明确推荐（"如果你是XX，就选YY，因为ZZ"）。`;
  ...
  if (type === "solve") {
    return `【本篇为「问题解决型」内容——用户遇到具体痛点，要结论+可执行步骤】
结构要求：
